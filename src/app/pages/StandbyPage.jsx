...
export default function Page({tag}) {
    ...
    console.log('findTopbarTitle: ', findTopbarTitle);
    // # 큰 주제의 다른 요소
    const anotherLinks = categoryList.filter(item => item.tag !== tag);
    console.log('anotherLinks: ', anotherLinks);

    // # 타이틀인 요소
    const findTopbarTwoStep = findTopbarTitle.children[pageNum - 1];
    // # 타이틀이 아닌 하단 요소 목록 찾기
    const findTopbarTitleLinksAnother = findTopbarTitle.children.filter((_,idx) => idx !== (pageNum - 1));
    console.log('findTopbarTitleLinksAnother: ', findTopbarTitleLinksAnother);
    const topbarList = [
        {
    ...
    if(findTopbarTwoStep.toggle && findTopbarTwoStep.toggle.length > 0) {
        console.log('findTopbarTwoStep.toggle: ', findTopbarTwoStep.toggle);
    ...
    const ViewPage = () => {
        const Pages = {
            1: <Page1 topbarList={dynamicTopbarList} />,
            2: <Page2 topbarList={dynamicTopbarList} />,
            3: <Page3 topbarList={dynamicTopbarList} />,
            4: <Page4 topbarList={dynamicTopbarList} />,
            5: <Page5 topbarList={dynamicTopbarList} />,
        };
