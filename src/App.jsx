...
import {
  ...
  AppEndBannerComponent,
} from './app/components/0_main/AppBanner.jsx';
...
// ~ Package Settings
const wgs84 =
  '+proj=longlat +datum=WGS84 +no_defs'; /* WGS84 좌표계 (기본 설정) */
const epsg2097 =
  '+proj=tmerc +lat_0=38 +lon_0=127 +k=1 +x_0=200000 +y_0=500000 +ellps=GRS80 +units=m +no_defs'; /* EPSG:2097 (Korea Central Belt / Korean 1985) */
...
function App() {
          ...
          <>
            <AppMMOSelectWrapper flex align="left">
              <AppMMOSelect
                ...
                bg
                $width="180px"
                onChange={(e) => eventHandler('type', e.currentTarget.value)}
                value={returnType}
              >
            ...
            <AppMMOBorderDiv flex>
              <button
                onClick={(e) => eventHandler('selectInfo', 'air')}
                active={selectInfo === 'air' ? 'on' : 'off'}
              >
                대기/경보 정보
              </button>
              <button
                onClick={(e) => eventHandler('selectInfo', 'station')}
                active={selectInfo === 'station' ? 'on' : 'off'}
              >
                측정소 정보
              </button>
            </AppMMOBorderDiv>
      ...
      case 1:
        return (
          <>
            ...
            <AppMMOBorderDiv>
              <button
                onClick={forecastDateHandle}
                active={forecastDate === '0' ? 'on' : 'off'}
                data-date="0"
              >
                오늘
              </button>
  ...
  useEffect(() => {
    ...
    const fetchStationData = async ({ longitude, latitude }) => {
      ...
      try {
        ...
        const responseStation = await fetch(
          `https://localhost:3500/api/airkorea/station?x=${tmX}&y=${tmY}`
        );
        ...
        // # 가까운 측정소 (1)
        const stationData = stationsInfo.find(
          (item) => item.stationName === stations[0].stationName
        );
    ...
    if ('geolocation' in navigator) {
      if (!stationFetchBoolean) {
        ...
        if (
          window.confirm('현재위치 정보를 이용해서 측정소 정보를 보시겠습니까?')
        ) {
          ...
          const error = (err) => {
            ...
            alert(
              "현재위치 사용권한이 거부되어 '중구' 지역을 기준으로 데이터를 출력합니다.\n\n활성화: \n설정 > 개인 정보 보호 및 보안 > 사이트 설정"
            );
            changer('station', defaultStation);
          };
          ...
          alert(
            "현재위치를 사용하지 않아 '중구' 지역을 기준으로 데이터를 출력합니다."
          );
          changer('station', defaultStation);
        }
      }
      ...
      alert(
        "현재위치를 지원하지 않아 '중구' 지역을 기준으로 데이터를 출력합니다."
      );
      changer('station', defaultStation);
      changer('stationFetchBoolean', true);
    }
    ...
    if (TimeText) {
      return (
        ...
        >
          {custom ? (
            <span>
              {custom[0]} <strong>{custom[1]} 발표자료</strong>
            </span>
          ) : (
            <span>
              {TimeText.year}년 {TimeText.month}월 {TimeText.day}일{' '}
              <strong>{TimeText.hour}시</strong>
            </span>
          )}
          {refresh && (
            <AppTimeButtonStyle onClick={onClick}>새로고침</AppTimeButtonStyle>
          )}
        </AppTimeDiv>
      );
    } else {
  ...
  function BannerData() {
    ...
    if (text !== null && TimeText !== undefined) {
      ...
      filterAirInformation = text?.filter((item) => {
        return (
          (item.dataTime ===
            `${TimeText.year}-${TimeText.month}-${TimeText.day} ${target}시 발표` &&
            item.informCode === 'PM10') ||
          (item.dataTime ===
            `${TimeText.year}-${TimeText.month}-${
              TimeText.day - 1
            } 23시 발표` &&
            item.informCode === 'PM10')
        );
      ...
      if (length === 3) {
        ...
        const informCause =
          '○ [정보] 현재 공공데이터 업데이트 중입니다. 10분 내외에 출력될 예정입니다.';
        const informGrade =
        ...
        const first = {
          ...preset,
          informData: `${TimeText.year}-${TimeText.month}-${TimeText.day}`,
        };
        const mid = {
          ...preset,
          informData: `${TimeText.year}-${TimeText.month}-${String(
            Number(TimeText.day) + 1
          ).padStart(2, '0')}`,
        };
        const last = {
          ...preset,
          informData: `${TimeText.year}-${TimeText.month}-${String(
            Number(TimeText.day) + 2
          ).padStart(2, '0')}`,
        };
        ...
        filterAirInformation.splice(length, 0, {
          ...last,
          informData: `${last?.informData.slice(0, -2)}${String(
            Number(last?.informData.slice(-2)) + 1
          ).padStart(2, '0')}`,
          informCause:
            '○ [미세먼지] 모레 4등급(좋음, 보통, 나쁨, 매우나쁨) 예보는 17시에 발표되며, 모레 2등급(낮음, 높음) 예보는 주간예보를 참고하시기 바랍니다.',
        });
      ...
      result = filterAirInformation?.map((item, idx) => {
        ...
        // 05시 전일 경우 어제 데이터 사용
        const nowDate = `${TimeText.year}-${TimeText.month}-${
          notData
            ? String(Number(TimeText.day) - 1).padStart(2, '0')
            : TimeText.day
        }`;
        const two = `${TimeText.year}-${TimeText.month}-${
          notData
            ? TimeText.day
            : String(Number(TimeText.day) + 1).padStart(2, '0')
        }`;
        const three = `${TimeText.year}-${TimeText.month}-${
          notData
            ? String(Number(TimeText.day) + 1).padStart(2, '0')
            : String(Number(TimeText.day) + 2).padStart(2, '0')
        }`;
        let txt;
        if (itemDate === nowDate) {
          notData ? (txt = '어제') : (txt = '금일');
        } else if (itemDate === two) {
          notData ? (txt = '금일') : (txt = '내일');
        } else if (itemDate === three) {
          notData ? (txt = '내일') : (txt = '모레');
        }
  ...
  const DynamicComponent = () => {
    ...
    return (
      <Result
        Time={Time}
        standbyType={tapSelect === 1 && standbyType}
        forecastDate={tapSelect === 1 && forecastDate}
      />
    );
  };
            ...
            <AppMMOptionLayout>
              <AppMMOList>
                ...
                <li select={tapSelect === 1 ? 'on' : 'off'}>
                  <button onClick={() => tapSelectHandle(1)}>
                    오늘/내일/모레 대기정보
                  </button>
                </li>
                <li select={tapSelect === 2 ? 'on' : 'off'}>
                  <button onClick={() => tapSelectHandle(2)}>
                    실시간 기상정보
                  </button>
                </li>
                <li select={tapSelect === 3 ? 'on' : 'off'}>
                  <button onClick={() => tapSelectHandle(3)}>
                    오늘/내일 기상정보
                  </button>
                </li>
              </AppMMOList>
        ...
        <AppSecondSection>
          <AppSecondBanner>
            <div className="updateTime">
              ...
              <strong>
                {(TimeText &&
                  `${Number(TimeText.hour) < 5 ? '23' : TimeText.hour}:${
                    TimeText.minute
                  }`) ||
                  '00:00'}
              </strong>
